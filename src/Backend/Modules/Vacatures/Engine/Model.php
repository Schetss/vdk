...
namespace Backend\Modules\Vacatures\Engine;

use Backend\Core\Engine\Model as BackendModel;
use Backend\Core\Engine\Language;

/**
 * In this file we store all generic functions that we will be using in the Vacatures module
 *
...
class Model
{
    ...
    const QRY_DATAGRID_BROWSE_CATEGORIES =
        'SELECT c.id, c.title, COUNT(i.id) AS num_items, c.sequence
         FROM vacatures_categories AS c
         LEFT OUTER JOIN vacatures AS i ON c.id = i.category_id AND i.language = c.language
         WHERE c.language = ?
         GROUP BY c.id
    ...
    {
        BackendModel::get('database')->delete('vacatures', 'id = ?', (int) $id);
    }
    ...
    public static function deleteCategory($id)
    {
        ...
        if (!empty($item)) {
            ...
            $db->delete('vacatures_categories', 'id = ?', array((int) $id));
            $db->update('vacatures', array('category_id' => null), 'category_id = ?', array((int) $id));
        }
    }
    ...
    public static function existsCategory($id)
    {
        return (bool) BackendModel::get('database')->getVar(
            'SELECT 1
             FROM vacatures_categories AS i
        ...
        if ($includeCount) {
            return (array) $db->getPairs(
                'SELECT i.id, CONCAT(i.title, " (",  COUNT(p.category_id) ,")") AS title
                 FROM vacatures_categories AS i
                 LEFT OUTER JOIN vacatures AS p ON i.id = p.category_id AND i.language = p.language
                 WHERE i.language = ?
                 GROUP BY i.id',
    ...
    /**
     * Get the maximum Vacatures sequence.
     *
     * @return int
     */
    ...
    public static function insert(array $item)
    {
        ...
        return (int) BackendModel::get('database')->insert('vacatures', $item);
    }
    ...
    public static function insertCategory(array $item)
    {
        ...
        return BackendModel::get('database')->insert('vacatures_categories', $item);
    }

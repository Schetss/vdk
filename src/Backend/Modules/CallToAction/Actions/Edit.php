...
class Edit extends ActionEdit
{
    ...
    protected function loadData()
    {
        ...
        if ($this->id == null || !BackendCallToActionModel::exists($this->id)) {
            $this->redirect(
                Model::createURLForAction('Index') . '&error=non-existing'
            );
        }

        $this->record = BackendCallToActionModel::get($this->id);
    }
    ...
    protected function loadForm()
    {
        ...
        // get categories
        $categories = BackendCallToActionModel::getCategories();
        $this->frm->addDropdown('category_id', $categories, $this->record['category_id']);

        // meta
        $this->meta = new Meta($this->frm, $this->record['meta_id'], 'titel', true);
        $this->meta->setUrlCallBack('Backend\Modules\CallToAction\Engine\Model', 'getUrl', array($this->record['id']));
        ...
        if ($this->frm->isSubmitted()) {
            ...
            if ($this->frm->isCorrect()) {
                ...
                if (!\SpoonDirectory::exists($imagePath . '/400x300')) {
                 ...
                 if (!\SpoonDirectory::exists($imagePath . '/400x200')) {
                    \SpoonDirectory::create($imagePath . '/400x200');
                }

...
class Model
{
    ...
    const QRY_DATAGRID_BROWSE_CATEGORIES =
        'SELECT c.id, c.title, COUNT(i.id) AS num_items, c.sequence
         FROM over_ons_categories AS c
         LEFT OUTER JOIN over_ons AS i ON c.id = i.category_id AND i.language = c.language
         WHERE c.language = ?
         GROUP BY c.id
    ...
    public static function deleteCategory($id)
    {
        ...
        if (!empty($item)) {
            ...
            $db->delete('over_ons_categories', 'id = ?', array((int) $id));
            $db->update('over_ons', array('category_id' => null), 'category_id = ?', array((int) $id));
        }
    }
        ...
        if ($includeCount) {
            return (array) $db->getPairs(
                'SELECT i.id, CONCAT(i.title, " (",  COUNT(p.category_id) ,")") AS title
                 FROM over_ons_categories AS i
                 LEFT OUTER JOIN over_ons AS p ON i.id = p.category_id AND i.language = p.language
                 WHERE i.language = ?
                 GROUP BY i.id',
    ...
    public static function insertCategory(array $item)
    {
        ...
        return BackendModel::get('database')->insert('over_ons_categories', $item);
    }

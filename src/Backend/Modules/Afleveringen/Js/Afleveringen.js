...
/**
 * Interaction for the Afleveringen module
 *
 * @author Stijn Schets <stijn@schetss.be>
 */
jsBackend.afleveringen =
{
    // constructor
    init: function()
    {
        ...
        if ($('#uploadedImages').length == 1) jsBackend.afleveringen.uploadHandler.init();
    },
    ...
    uploadHandler:
    {
        ...
        init: function()
        {
            ...
            if ($('#uploadedImages').val())
            {
                ...
                jsBackend.afleveringen.uploadHandler.buildFileList();
            }

            // bind complete callback
            uploader.element.on('upload', function(event, id, fileName)
            {
                jsBackend.afleveringen.uploadHandler.lowestSequence--;
                uploader.uploadedFiles[id] = {
                    ...
                    warning: '',
                    sequence: jsBackend.afleveringen.uploadHandler.lowestSequence,
                    progress: 0
                };
            ...
            $('#jsFileList').sortable({
                update: function(event, ui){ jsBackend.afleveringen.uploadHandler.sortImages($(this)); },
                tolerance: 'pointer'
            });
        },
        ...
        buildFileList: function()
        {
            if ($('#jsFileList').length > 0)
            {
                ...
                var orderedKeys = jsBackend.afleveringen.uploadHandler.getOrderedKeys();
                ...
                $('.jsDeleteImage').on('click', jsBackend.afleveringen.uploadHandler.deleteImage );
            }
        },
        ...
        sortImages: function($items)
        {
            ...
            jsBackend.afleveringen.uploadHandler.lowestSequence = 0;
            $('#uploadedImages').val(JSON.stringify(uploader.uploadedFiles));
        }

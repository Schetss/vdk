...
namespace Backend\Modules\CallToAction\Engine;
...
class Model
{
    ...
    const QRY_DATAGRID_BROWSE_CATEGORIES =
        'SELECT c.id, c.title, COUNT(i.id) AS num_items, c.sequence
         FROM call_to_action_categories AS c
         LEFT OUTER JOIN call_to_action AS i ON c.id = i.category_id AND i.language = c.language
         WHERE c.language = ?
         GROUP BY c.id
    ...
    {
        BackendModel::get('database')->delete('call_to_action', 'id = ?', (int) $id);
    }
    ...
    public static function deleteCategory($id)
    {
        ...
        if (!empty($item)) {
            ...
            $db->delete('call_to_action_categories', 'id = ?', array((int) $id));
            $db->update('call_to_action', array('category_id' => null), 'category_id = ?', array((int) $id));
        }
    }
        ...
        if ($includeCount) {
            return (array) $db->getPairs(
                'SELECT i.id, CONCAT(i.title, " (",  COUNT(p.category_id) ,")") AS title
                 FROM call_to_action_categories AS i
                 LEFT OUTER JOIN call_to_action AS p ON i.id = p.category_id AND i.language = p.language
                 WHERE i.language = ?
                 GROUP BY i.id',
    ...
    /**
     * Get the maximum Call To Action sequence.
     *
     * @return int
     */
    ...
    public static function insert(array $item)
    {
        ...
        return (int) BackendModel::get('database')->insert('call_to_action', $item);
    }
    ...
    public static function insertCategory(array $item)
    {
        ...
        return BackendModel::get('database')->insert('call_to_action_categories', $item);
    }

...
class EditCategory extends ActionEdit
{
    ...
    private function getData()
    {
        ...
        if ($this->id == null || !BackendOverOnsModel::existsCategory($this->id)) {
            $this->redirect(
                Model::createURLForAction('categories') . '&error=non-existing'
            );
        }

        $this->record = BackendOverOnsModel::getCategory($this->id);
    }
    ...
    private function loadForm()
    {
        ...
        $this->meta->setUrlCallback('Backend\Modules\OverOns\Engine\Model', 'getURLForCategory', array($this->record['id']));
    }

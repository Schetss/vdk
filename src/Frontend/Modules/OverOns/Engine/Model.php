...
class Model
{
    ...
    public static function getAll($limit = 10, $offset = 0)
    {
        $items = (array) FrontendModel::get('database')->getRecords(
            'SELECT id, titel as titel, year(datum) as datum, afbeelding, tekst as tekst
             FROM over_ons 
             ORDER BY datum DESC',
    ...
    public static function getAllCategories()
    {
        $return = (array) FrontendModel::get('database')->getRecords(
            'SELECT c.id, c.title AS label, m.url, COUNT(c.id) AS total, m.data AS meta_data
             ...
             INNER JOIN over_ons AS i ON c.id = i.category_id AND c.language = i.language
    ...
    public static function getCategory($URL)
    {
        ...
        // create full url
        $item['full_url'] = Navigation::getURLForBlock('OverOns', 'category') . '/' . $item['url'];

...
namespace Backend\Modules\Producten\Engine;

use Backend\Core\Engine\Model as BackendModel;
use Backend\Core\Engine\Language;

/**
 * In this file we store all generic functions that we will be using in the Producten module
 *
...
class Model
{
    ...
    const QRY_DATAGRID_BROWSE_CATEGORIES =
        'SELECT c.id, c.title, COUNT(i.id) AS num_items, c.sequence
         FROM producten_categories AS c
         LEFT OUTER JOIN producten AS i ON c.id = i.category_id AND i.language = c.language
         WHERE c.language = ?
         GROUP BY c.id
    ...
    {
        BackendModel::get('database')->delete('producten', 'id = ?', (int) $id);
    }
    ...
    public static function deleteCategory($id)
    {
        ...
        if (!empty($item)) {
            ...
            $db->delete('producten_categories', 'id = ?', array((int) $id));
            $db->update('producten', array('category_id' => null), 'category_id = ?', array((int) $id));
        }
    }
    ...
    public static function existsCategory($id)
    {
        return (bool) BackendModel::get('database')->getVar(
            'SELECT 1
             FROM producten_categories AS i
        ...
        if ($includeCount) {
            return (array) $db->getPairs(
                'SELECT i.id, CONCAT(i.title, " (",  COUNT(p.category_id) ,")") AS title
                 FROM producten_categories AS i
                 LEFT OUTER JOIN producten AS p ON i.id = p.category_id AND i.language = p.language
                 WHERE i.language = ?
                 GROUP BY i.id',
    ...
    /**
     * Get the maximum Producten sequence.
     *
     * @return int
     */
    ...
    public static function insert(array $item)
    {
        ...
        return (int) BackendModel::get('database')->insert('producten', $item);
    }
    ...
    public static function insertCategory(array $item)
    {
        ...
        return BackendModel::get('database')->insert('producten_categories', $item);
    }
